...
# PDF generation functions that return bytes instead of response
...
def fill_att_accident_pdf(d):
    ...
    if d.get('date_signature'): txt(c,d['date_signature'],237,443)
    c.save()
    return merge(TEMPLATES['att_accident'],p,1).getvalue()

def fill_att_seppt_pdf(d):
    p,c=newcan()
    if d.get('nom_soussigne'): txt(c,d['nom_soussigne'],313,129)
    if d.get('niss'): txt(c,d['niss'],313,145)
    if d.get('domicile_1'): txt(c,d['domicile_1'],310,160)
    ...
    if d.get('date_signature'): txt(c,d['date_signature'],252,548)
    c.save()
    return merge(TEMPLATES['att_seppt'],p,1).getvalue()
...
def send_for_signature():
    """
    Generate PDFs and send them to Yousign for signature
    Expected JSON:
    {
        "signer_email": "client@example.com",
        "signer_name": "John Doe",
        "signer_phone": "+32470123456",  # optional
        "documents": ["employer", "att_accident", "offre1", ...],
    ...
    try:
        ...
        if not signer_email or not signer_name:
            return jsonify({"error": "signer_email and signer_name are required"}), 400
        ...
        signature_request_payload = {
            "name": f"Documents PersoProject - {signer_name}",
            "delivery_mode": "email",
        ...
        if sig_response.status_code != 201:
            return jsonify({"error": f"Failed to create signature request: {sig_response.text}"}), 500
        
        signature_request = sig_response.json()
        ...
        signer_payload = {
            "info": {
                ...
                "last_name": signer_name.split()[-1] if ' ' in signer_name else signer_name,
                "email": signer_email,
                "locale": "fr"
            },
        ...
        if signer_phone:
            signer_payload["info"]["phone_number"] = signer_phone
        ...
        if signer_response.status_code != 201:
            return jsonify({"error": f"Failed to add signer: {signer_response.text}"}), 500
        ...
        for doc in uploaded_documents:
            ...
            field_response = requests.post(
                f'{YOUSIGN_API_URL}/signature_requests/{signature_request_id}/documents/{doc["id"]}/fields',
                headers=headers,
                json=field_payload
            )
        ...
        return jsonify({
            "success": True,
            "message": f"Documents envoyés à {signer_email} pour signature",
